//! Request-scoped state for live Zsh completion capture.
//!
...
use sense_model::{
    CompletionItem, CompletionKind, CompletionResource, Confidence, DocumentationState, Generation,
    GroupId, ItemCapabilities, ItemId, ItemTags, NativeShell, RawBytes, RequestId, SourceId,
    TextEdit, TextRange,
};
...
const CAPTURE_FINGERPRINT_DOMAIN: &[u8] = b"shell-sense/zsh-capture-fingerprint/v2";
const ITEM_ID_DOMAIN: &[u8] = b"shell-sense/stable-zsh-item/v1";

bitflags! {
    /// Subset of Zsh match behavior required for presentation and acceptance
    /// diagnostics. The live provider retains its complete replay record in
...
pub struct CapturedMatch {
    ...
    /// Opaque replay token, meaningful only to the live provider generation
...
pub enum CaptureError {
    ...
    InvalidLimits,
    #[error("the generic shell capture store cannot be used for Zsh")]
    InvalidShellStore,
    ...
    #[error("the selected fingerprint is not present in the active capture")]
    UnknownFingerprint,
}

/// Data required by the live Zsh provider to replay acceptance.
...
impl CaptureStore {
    ...
    pub fn install(
        ...
        for (index, captured) in matches.into_iter().enumerate() {
            ...
            let fingerprint = capture_fingerprint(request, ordinal, &captured);
            ...
            routes.insert(
                ...
                AcceptanceRoute {
                    ...
                    acceptance_identity: captured.acceptance_identity,
                },
            );
            items.push(item);
        }
    ...
    /// Resolve a daemon selection to the live provider record.
    ///
    /// # Errors
    ///
...
fn completion_item(
    ...
    // Native descriptions already appear beside the label in the completion
    // menu. Repeating them in the documentation pane adds no information;
    // that pane is reserved for independently resolved documentation.
    let documentation = DocumentationState::None;
    ...
    .into_completion_item(NativeShell::Zsh)
}
...
fn capture_fingerprint(
    ...
    hash_bytes(&mut hash, captured.acceptance_identity.as_slice());
...
fn capture_size(captured: &CapturedMatch) -> usize {
    ...
    [
        ...
        captured.acceptance_identity.len(),
        metadata.prefix.len(),
        metadata.suffix.len(),
        ...
        captured.resource_path.as_ref().map_or(0, RawBytes::len),
