...
typeset -gr sense_binary=$project_root/target/debug/shell-sense
typeset -g test_dir
test_dir=$(mktemp -d)
...
"$sense_binary" worker --socket "$test_dir/daemon.sock" \
  ...
  --no-daemon-autostart --shell zsh --shell-executable "${commands[zsh]}" \
  --shell-version "$ZSH_VERSION" --shell-patchlevel "$ZSH_PATCHLEVEL" \
...
source "$project_root/shell/zsh/client.zsh"
_shell_sense_rebuild_styles
[[ $_shell_sense_style_label == 'fg=#d4d4d4,bg=#202020' &&
   $_shell_sense_style_label_selected == 'fg=#d4d4d4,bg=#343b41' &&
   $_shell_sense_style_label_match_selected == 'fg=#18a2fe,bg=#343b41,bold' &&
   $_shell_sense_style_scrollbar_thumb == 'fg=#bbbbbb,bg=#202020' &&
   $_shell_sense_style_scrollbar_gutter == 'fg=#343b41,bg=#202020' &&
   $_shell_sense_style_ghost == 'fg=#707070' ]] || {
...
_shell_sense_ghost_chunk 'files/nvim/init.lua' path-segment
[[ $REPLY == 'files/' ]] || {
...
_shell_sense_rx_buffer=$stream
typeset -gi parse_status=0
_shell_sense_parse_messages || parse_status=$?
[[ -z $_shell_sense_rx_buffer && $parsed_commands[1] == ready &&
   ${parsed_commands[(Ie)popup-option]} -gt 0 &&
   ${parsed_commands[(Ie)ghost-config]} -gt 0 &&
   $parsed_commands[-1] == config-end ]] || {
  print -u2 -- "Zsh could not parse the startup stream (status $parse_status, parsed ${(j:,:)parsed_commands}): ${(qqq)_shell_sense_rx_buffer}"
  return 1
}
