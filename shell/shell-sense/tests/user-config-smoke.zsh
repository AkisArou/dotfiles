...
typeset -gx SHELL_SENSE_SOCKET="$test_temp/daemon.sock"
typeset -gx SHELL_SENSE_CONFIG="$project_root/config.example.toml"
typeset -gx SHELL_SENSE_COMMAND=${SHELL_SENSE_COMMAND:-"$project_root/target/release/shell-sense"}
...
cleanup() {
  ...
  if [[ ${SHELL_SENSE_KEEP_TEST_TEMP:-0} != 1 && -d $test_temp && $test_temp == /tmp/tmp.* ]]; then
...
"$SHELL_SENSE_COMMAND" daemon --socket "$SHELL_SENSE_SOCKET" \
  --config "$SHELL_SENSE_CONFIG" >"$test_temp/daemon.log" 2>&1 &
daemon_pid=$!
for _ in {1..200}; do
...
# zsh-autosuggestions owns Ctrl-E before shell-sense loads. The state-aware
...
output=
read_until '*.shell-sense-key-ctrl-e*' || {
  print -u2 -- 'Ctrl-E was not bound to the shell-sense dispatcher'
  print -u2 -r -- "$output"
  return 1
}
