...
fn captured(insertion: impl Into<RawBytes>) -> CapturedMatch {
    CapturedMatch {
        ...
        backend_identity: RawBytes::from("backend-record-1"),
        original_order: 0,
    }
}
...
fn capture_preserves_raw_insertion_and_routes_acceptance() {
    ...
    let outcome = store
        .install(
            ...
            vec![captured(vec![b'r', b'e', 0xff, b't'])],
        )
        .expect("capture");
    ...
    let InsertStrategy::ZshMatch { fingerprint } = &item.insertion else {
        panic!("expected Zsh insertion identity");
    };
    let route = store
        .acceptance(request.request_id, request.generation, fingerprint)
        .expect("live route");
    assert_eq!(route.backend_identity.as_slice(), b"backend-record-1");
...
fn display_controls_are_escaped_before_reaching_the_ui() {
    ...
    let mut store = CaptureStore::new(CaptureLimits::default()).expect("valid limits");
    let outcome = store
        .install(&request, CaptureBackend::Portable, vec![candidate])
        .expect("capture");
...
fn a_new_generation_invalidates_old_acceptance_fingerprints() {
    ...
    let first_outcome = store
        .install(&first, CaptureBackend::Native, vec![captured("restart")])
        .expect("capture");
    let InsertStrategy::ZshMatch { fingerprint } = &first_outcome.batch.items[0].insertion else {
        panic!("expected fingerprint");
    };
    let stable_id = first_outcome.batch.items[0].id.clone();

    let second_outcome = store
        .install(&second, CaptureBackend::Native, vec![captured("restart")])
        .expect("capture");
...
fn invalid_edit_ranges_reject_the_capture() {
    ...
    assert_eq!(
        store.install(&request, CaptureBackend::Native, vec![candidate]),
