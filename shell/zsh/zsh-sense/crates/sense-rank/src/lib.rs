...
//! never interprets or reconstructs Zsh insertion fingerprints.
...
use sense_model::{
    CompletionItem, CompletionKind, Confidence, DocumentationState, GroupId, InsertStrategy,
    ItemId, MatchResult, RawBytes, SourceId, TextRange,
};

const SCORE_SCALE: i64 = 1_024;
const EXACT_BOOST: i64 = 100_000_000;
const PREFIX_BOOST: i64 = 10_000_000;
const INSERTION_AUTHORITY_BOOST: i64 = 1_000_000;
...
fn dedup_key(item: &CompletionItem, preserve_groups: bool) -> DedupKey {
    let insertion = match &item.insertion {
        InsertStrategy::Snippet { body } => InsertionIdentity::Snippet(body.clone()),
        InsertStrategy::TextEdit | InsertStrategy::ZshMatch { .. }
            if !item.edit.new_text.is_empty() =>
        {
...
fn contextual_score(item: &CompletionItem, signals: &RankSignals) -> i64 {
    i64::from(
        ...
        + if matches!(item.insertion, InsertStrategy::ZshMatch { .. }) {
            INSERTION_AUTHORITY_BOOST
        } else {
            0
        }
}

fn primary_score(item: &CompletionItem, signals: &RankSignals) -> (u8, u8, i64, u8) {
    (
        u8::from(matches!(item.insertion, InsertStrategy::ZshMatch { .. })),
