...
# Export patterns so that they can be tested in pattern-check
declare vim_pattern='(\S+/)?g?\.?(view|l?n?vim?x?|fzf)(diff)?(-wrapped)?'
declare emacs_pattern='(\S+/)?e?macs(client)?(-\w+)?'

bind_key_editor() {
    local key tmux_cmd is_vim is_emacs is_editor
    key="$1"
    tmux_cmd="$2"
    
    vim_pattern="$(get_tmux_option "@vim_navigator_pattern" "${vim_pattern}")"
    emacs_pattern="$(get_tmux_option "@emacs_navigator_pattern" "${emacs_pattern}")"
    ...
    is_emacs="ps -o state= -o comm= -t '#{pane_tty}' \
        | grep -iqE '^[^TXZ ]+ +@emacs_navigator_pattern$'"
    is_emacs="$(get_tmux_option "@emacs_navigator_check" "${is_emacs}")"
    is_emacs="${is_emacs//@emacs_navigator_pattern/${emacs_pattern}}"
    
    # Combine checks: run if either vim or emacs is active
    is_editor="$is_vim || $is_emacs"
    
    # sending C-/ according to https://github.com/tmux/tmux/issues/1827
    tmux bind-key -n "$key" if-shell "$is_editor" "send-keys '$key'" "$tmux_cmd"
...
main() {
    ...
    for k in $(echo "$move_left"); do
        bind_key_editor "$k" "select-pane -L"
    done
    
    for k in $(echo "$move_down"); do
        bind_key_editor "$k" "select-pane -D"
    done
    
    for k in $(echo "$move_up"); do
        bind_key_editor "$k" "select-pane -U"
    done
    
    for k in $(echo "$move_right"); do
        bind_key_editor "$k" "select-pane -R"
    done
    
    for k in $(echo "$move_prev"); do
        bind_key_editor "$k" "select-pane -l"
    done
    
    # Restoring clear screen
    clear_screen="$(get_tmux_option "@vim_navigator_prefix_mapping_clear_screen" 'C-l')"
    for k in $(echo "$clear_screen"); do
        tmux bind "$k" send-keys 'C-l'
    done
}
