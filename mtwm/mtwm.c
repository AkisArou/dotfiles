...
#define MOD XCB_MOD_MASK_1 // Super key
...
Workspace *create_workspace(int num, int focused) {
...
Screen *create_screen() {
  ...
  Workspace *workspace = create_workspace(0, -1);
  screen->default_workspace = workspace;
  screen->curr_workspace = workspace;

  screen->workspace_count = 1;
  screen->workspaces = malloc(sizeof(Workspace *) * screen->workspace_count);
...
void destroy_workspace(Screen *screen, Workspace *workspace) {
  for (int i = 0; i < screen->workspace_count; i++) {
    ...
    for (int j = 0; j < workspace->windows_count; j++) {
...
void update_focus_borders() {
  ...
  for (int i = 0; i < workspace->windows_count; ++i) {
    ...
    uint32_t color = (i == workspace->focused) ? FOCUSED_BORDER_COLOR
...
void tile_windows() {
  ...
  int available_width =
      screen_width - (GAP * 2) - (GAP * (workspace->windows_count - 1));
...
void create_window(xcb_window_t win) {
  ...
  workspace->windows = realloc(
      workspace->windows, sizeof(Window *) * (workspace->windows_count + 1));
...
void resize_focused(float delta) {
  ...
  if (workspace->windows_count == 0 || workspace->focused < 0)
    return;
  workspace->windows[workspace->focused]->width_factor += delta;
  tile_windows();
}
...
void move_focused_left() {
  ...
  workspace->windows[workspace->focused] =
      workspace->windows[workspace->focused - 1];

  workspace->windows[workspace->focused - 1] = tmp;
  workspace->focused--;
  tile_windows();
}
...
void grab_keys() {
  ...
  xcb_keycode_t return_key =
      *xcb_key_symbols_get_keycode(syms, 0xFF0D); // XK_Return

  // Focus left/right
  xcb_grab_key(conn, 1, scr->root, MOD, h_key, XCB_GRAB_MODE_ASYNC,
               XCB_GRAB_MODE_ASYNC); // MOD+h
  xcb_grab_key(conn, 1, scr->root, MOD, l_key, XCB_GRAB_MODE_ASYNC,
               XCB_GRAB_MODE_ASYNC); // MOD+l
  // Move window left/right
  xcb_grab_key(conn, 1, scr->root, MOD | XCB_MOD_MASK_SHIFT, h_key,
               XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC); // MOD+Shift+h
  xcb_grab_key(conn, 1, scr->root, MOD | XCB_MOD_MASK_SHIFT, l_key,
               XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC); // MOD+Shift+l
  // Resize left/right
  xcb_grab_key(conn, 1, scr->root, MOD | XCB_MOD_MASK_CONTROL, h_key,
               XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC); // MOD+Ctrl+h
  xcb_grab_key(conn, 1, scr->root, MOD | XCB_MOD_MASK_CONTROL, l_key,
               XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC); // MOD+Ctrl+l
  // Open terminal
  xcb_grab_key(conn, 1, scr->root, MOD, return_key, XCB_GRAB_MODE_ASYNC,
               XCB_GRAB_MODE_ASYNC); // MOD+Enter
  // Close focused window
  xcb_grab_key(conn, 1, scr->root, MOD, q_key, XCB_GRAB_MODE_ASYNC,
               XCB_GRAB_MODE_ASYNC); // MOD+q
  // Launch rofi
  xcb_grab_key(conn, 1, scr->root, MOD, d_key, XCB_GRAB_MODE_ASYNC,
               XCB_GRAB_MODE_ASYNC); // MOD+d
  ...
  while ((ev = xcb_wait_for_event(conn))) {
    ...
    case XCB_KEY_PRESS: {
      ...
      if (e->state & MOD) {
        ...
        if (keysym == 'h' && !(e->state & XCB_MOD_MASK_SHIFT) &&
        ...
        // Other actions (MOD only)
          ...
          if (workspace->focused >= 0 &&
              workspace->focused < workspace->windows_count) {
            xcb_destroy_window(conn,
                               workspace->windows[workspace->focused]->win);
            xcb_flush(conn);
          }
        } else if (keysym == 'd') {
          if (fork() == 0) {
            setsid(); // Start new session
            execlp("rofi", "/home/akisarou/dotfiles/rofi/launch", "-show",
                   "drun", NULL);
            _exit(1); // If execlp fails
          }
          xcb_flush(conn);
