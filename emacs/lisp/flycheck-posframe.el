...
(defvar flycheck-posframe-manually-triggered nil
  "Flag to indicate posframe was manually triggered.")

(defvar flycheck-posframe-visible nil
  "Flag to indicate if posframe is currently visible.")
...
(defun flycheck-posframe-highest-error-level-face (errs)
  ...
  (flycheck-posframe-get-face-for-error
   (cl-reduce
	(lambda (err1 err2)
	  (if (flycheck-error-level-< err1 err2) err2 err1))
	errs)))
...
(defun flycheck-posframe-hidehandler (_info)
  "Don't auto-hide posframe - we handle hiding manually."
  nil)

(defun flycheck-posframe-show-posframe (errors)
  "Store ERRORS but don't display automatically."
  ;; Don't auto-hide if manually visible
  (unless flycheck-posframe-visible
	(posframe-hide flycheck-posframe-buffer)))

(defun flycheck-posframe-display-errors-manually ()
  "Manually display flycheck errors at point."
  (interactive)
	...
	(if (and errors
		...
		(let ((poshandler (intern (format "posframe-poshandler-%s" flycheck-posframe-position))))
		  (unless (functionp poshandler)
			(setq poshandler nil))
		  (setq flycheck-posframe-last-position (point))
		  (setq flycheck-posframe-manually-triggered t)
		  (setq flycheck-posframe-visible t)
		  (posframe-show
	  ...
	  (flycheck-posframe-hide))))

(defun flycheck-posframe-hide ()
  "Hide the flycheck posframe immediately."
  (setq flycheck-posframe-visible nil)
  (posframe-hide flycheck-posframe-buffer))

(defun flycheck-posframe-hide-on-movement ()
  "Hide posframe when cursor moves."
  (cond
   ;; Just triggered manually, reset flag and don't hide
   (flycheck-posframe-manually-triggered
	(setq flycheck-posframe-manually-triggered nil))
   ;; Cursor moved, hide the posframe
   ((not (equal (point) flycheck-posframe-last-position))
	(flycheck-posframe-hide)
	(setq flycheck-posframe-last-position (point)))))

(defun flycheck-posframe-hide-on-insert ()
  "Hide posframe when entering insert mode."
  (flycheck-posframe-hide))
  ...
  (cond
   ((and flycheck-posframe-mode
	...
	(setq-local flycheck-display-errors-function
				#'flycheck-posframe-show-posframe)
	(add-hook 'post-command-hook #'flycheck-posframe-hide-on-movement nil t)
	(when (featurep 'evil)
	  (add-hook 'evil-insert-state-entry-hook #'flycheck-posframe-hide-on-insert nil t)))
   ((and (not flycheck-posframe-mode)
	...
	(remove-hook 'post-command-hook #'flycheck-posframe-hide-on-movement t)
	(when (featurep 'evil)
	  (remove-hook 'evil-insert-state-entry-hook #'flycheck-posframe-hide-on-insert t))
	(flycheck-posframe-hide))))
